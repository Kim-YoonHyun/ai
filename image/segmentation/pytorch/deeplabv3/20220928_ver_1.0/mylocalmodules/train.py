...
def make_confusion_matrix(class_list, true, pred, save=None):
    ...
    acc_report, new_class_list = make_acc_report(class_list=class_list, true=true, pred=pred)
...
def train(model, start_epoch, epochs, train_dataloader, validation_dataloader, 
    ...
    for epoch in range(start_epoch, epochs):
        ...
        with torch.no_grad():
            ...
            # 이미지별 pixel confusion matrix
            # true_label_ary = np.reshape(true_label_ary, (true_label_ary.shape[0], -1))
            # pred_label_ary = np.reshape(pred_label_ary, (pred_label_ary.shape[0], -1))
            # val_acc_sum = 0
            # for true_label, pred_label in zip(true_label_ary, pred_label_ary):
            #     val_confusion_matrix = make_confusion_matrix(
            #         class_list=class_list,
            #         true=true_label,
            #         pred=pred_label)
            #     val_acc = val_confusion_matrix['accuracy'].values[0]
            #     val_acc_sum += val_acc
            # val_acc = val_acc_sum / len(pred_label_ary)
    ...
    utils.save_csv(save_path=f'{model_save_path}/epoch{str(epoch + 1).zfill(4)}/confusion_matrix.csv',
                    data_for_save=val_confusion_matrix)
