...
class VnAW(nn.Module):
    def __init__(self, x_len, y_len, d_model, d_ff, n_heads, 
        ...
        if self.projection_type == 'normal':
            self.conv = nn.Conv1d(in_channels=y_len, out_channels=y_len, kernel_size=1)
            self.activation = nn.ReLU()
            self.dropout = nn.Dropout(dropout_p)
            self.linear = nn.Linear(d_model, output_length, bias=True)
            
        if self.projection_type == 'res_conv':
            self.conv1 = nn.Conv1d(in_channels=y_len, out_channels=y_len, kernel_size=1)
            self.activation1 = nn.ReLU()
            self.conv2 = nn.Conv1d(in_channels=y_len, out_channels=y_len, kernel_size=1)
