...
def tester(test_args_setting):
    ...
    # norm 불러오기
    norm_path = '/'.join(test_args_setting['test_data'].split('/')[:-2])
    ...
    sensor_norm_num = norm[sensor_name]
    ...
    result_df.to_csv(f'{test_args_setting["root_path"]}/result_df_{sensor_name}.csv', index=False, encoding='utf-8-sig')
