...
class AnomalyDataset(Dataset):
    ...
    def __init__(self, x_df_list, mark_df_list, pred_len):
        self.x_df_list = x_df_list
        self.mark_df_list = mark_df_list
        self.pred_len = pred_len

        # ==
        a = mark_df_list[0].iloc[:, :1].values
        a = np.squeeze(a)
        self.get_self_mask(a)
        # ==
    ...
    def __getitem__(self, idx):
        ...
        x_ = np.squeeze(x_)
        self_mask, look_ahead_mask = self.get_self_mask(x_)
        mark_df = self.mark_df_list[idx]
        
        x = x_df.to_numpy()
        x_mark = mark_df.to_numpy()

        return x, x_mark, self_mask, look_ahead_mask
