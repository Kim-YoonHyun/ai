...
class EncoderBlock(nn.Module):
    ...
    def forward(self, input, mask):
        ...
        x_m_h_attn, attn_score = self.Multi_Head_Attention(
            query=x_norm, 
            key=x_norm, 
            value=x_norm, 
            mask=mask
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, target, encoder_output, target_mask, encoder_mask):
        ...
        x_masked_m_h_attn, _ = self.Multi_Head_Attention_1(
            query=x_norm, 
            key=x_norm, 
            value=x_norm, 
            mask=target_mask
        ...
        x_masked_m_h_attn, _ = self.Multi_Head_Attention_2(
            query=x_norm, 
            key=enc_output_norm, 
...
class Classifier(nn.Module):
    def __init__(self, input_size, output_size, dropout_p=0.2):
        super().__init__()
        ...
        self.linear_4 = nn.Linear(32, 4)
        self.linear_5 = nn.Linear(512*4, 1024)
        self.linear_6 = nn.Linear(1024, output_size)
        self.relu = nn.ReLU()
...
class Transformer(nn.Module):
    def __init__(self, num_embeddings, d_model, max_length, head_num, dropout_p, layer_num):
        super().__init__()
        ...
        self.classifier = Classifier(num_embeddings, max_length, dropout_p)
    ...
    def forward(self, input, target, input_mask, target_mask):
        ...
        for encoder_block in self.encoder:
            x_enc, attn_score = encoder_block(x_enc, input_mask)
        
        tar_emb = self.embedding(target)
        tar_enc = self.positional_encoding(tar_emb)

        for decoder_block in self.decoder:
            tar_enc, attn_score = decoder_block(tar_enc, x_enc, target_mask, input_mask)
    ...
    def encode(self,input, input_mask):
        ...
        for encoder in self.encoder:
            x_enc, _ = encoder(x_enc, input_mask)
    ...
    def decode(self, encode_output, encoder_mask, target, target_mask):
        ...
        #target, encoder_output, target_mask, encoder_mask
            target, _ = decoder(target, encode_output, target_mask, encoder_mask)
