...
class LossFunction(nn.Module):
    ...
    def forward(self, pred, b_label):
        # pred = pred.float()
        # b_label = b_label.float()*0.001
        # loss = self.loss_function(pred, b_label)
...
def main():
    ...
    # input_list = []
    # label_list = []
    target_path = f'{root_path}/datasets/{dataset_name}/{group}'
    target_name_list = os.listdir(target_path)
    target_name_list.sort()
    
    for target_name in target_name_list:
        
        print(f'{target_name} read data...')
        
        # 개별 입력
        train_name_list = os.listdir(f'{target_path}/{target_name}/train')
        test_name_list = os.listdir(f'{target_path}/{target_name}/test')
        
        
        # # 전체 입력
        # train_input_df = pd.read_csv(f'{target_path}/{target_name}/train/input/whole_data.csv', encoding='utf-8-sig')
        # train_label_df = pd.read_csv(f'{target_path}/{target_name}/train/label/whole_data.csv', encoding='utf-8-sig')
        # test_input_df = pd.read_csv(f'{target_path}/{target_name}/test/input/whole_data.csv', encoding='utf-8-sig')
        # test_label_df = pd.read_csv(f'{target_path}/{target_name}/test/label/whole_data.csv', encoding='utf-8-sig')
        # train_input_list = train_input_df.values.tolist()
        # train_label_list = train_label_df.values.tolist()
        # test_input_list = test_input_df.values.tolist()
        # test_label_list = test_label_df.values.tolist()
        
        #==
        # input_list = []
        # label_list = []
        # with open(f'{root_path}/datasets/{dataset_name}', encoding='utf-8-sig') as f:
        #     data_dict = json.load(f)
        # kor_list = data_dict['train']['kor_list'][:100]
        # en_list = data_dict['train']['eng_list'][:100]
        ...
        # 개별입력
        Train_Dataloader = dam.get_dataloader(
            dataset_path=f'{target_path}/{target_name}/train', 
            data_name_list=train_name_list,
            batch_size=batch_size,
            shuffle=shuffle,
            drop_last=drop_last,
            num_workers=num_workers,
        ...
        Val_Dataloader = dam.get_dataloader(
            dataset_path=f'{target_path}/{target_name}/test', 
            data_name_list=test_name_list,
            batch_size=batch_size,
            shuffle=shuffle,
            drop_last=drop_last,
            num_workers=num_workers,
        ...
        #     layer_memory = param.numel() * param.element_size()
        #     print(name, layer_memory)
        ...
        # ============================================================
        print('train...')
        root_save_path = f'{root_path}/trained_model/{dataset_name}/{group}/{target_name}'
