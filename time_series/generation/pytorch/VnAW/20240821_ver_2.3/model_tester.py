...
def tester(test_args_setting):
    ...
    i = 0
    for true_y, pred_y in zip(true_ary_list, logit_ary):
        sensor_norm_num = norm['ECU_DOCGasTemperature_After']
        true_y = np.squeeze(true_y)
        pred_y = np.squeeze(pred_y) * sensor_norm_num
        ...
        order = str(i).zfill(2)
        plm.draw_plot(
            title=f'{order}_result_{test_args_setting["trained_model"]}',
            x=range(len(true_y)),
            y=pred_y,
            title_font_size=20,
            x_font_size=20,
            ...
            add_y_list=[true_y],
            fig_size=(20, 5),
            save_path=f"{test_args_setting['trained_model_path']}"
        )
        i += 1
        # ==
