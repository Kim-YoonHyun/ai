...
if __name__ == "__main__":
        ...
        for xym in ['x', 'y', 'mark']:
            ...
            for name in tqdm(filter_name_list):
                if xym == 'mark':
                    if embed_type == 'pure':
                        continue
                df = pd.read_csv(f'{dataset_path}/{tv}/{xym}/{name}')
                ary = df.to_numpy()
                temp_list.append(ary)
                
                # self mask / enc-dec self mask
                if xym == 'x':
                    x_ary = ary.copy()
                    x_ = x_ary[:, :1]
                    x_ = np.squeeze(x_) 
                    self_mask, _ = dam.get_self_mask(x_, pred_len=x_len, n_heads=n_heads)
                    temp_self_mask_list.append(self_mask)   
                ...
                if xym == 'y':
                    ...
                    temp_la_mask_list.append(look_ahead_mask)
            
            temp_ary = np.array(temp_list.copy())
            data_dict[tv][xym] = temp_ary
        data_dict[tv]['self_mask'] = temp_self_mask_list
        data_dict[tv]['la_mask'] = temp_la_mask_list
    del name_list, filter_name_list, temp_list, temp_ary, 
    del temp_self_mask_list, temp_la_mask_list
    
    x_train_ary_ary = data_dict['train']['x']
    y_train_ary_ary = data_dict['train']['y']
    
    x_val_ary_ary = data_dict['val']['x']
    y_val_ary_ary = data_dict['val']['y']
    
    if embed_type == 'pure':
        mark_train_ary_ary = copy.deepcopy(x_train_ary_ary)
        mark_val_ary_ary = copy.deepcopy(x_val_ary_ary)
    else:
        mark_train_ary_ary = data_dict['train']['mark']
        mark_val_ary_ary = data_dict['val']['mark']
    
    train_self_mask_list = data_dict['train']['self_mask']
    train_la_mask_list = data_dict['train']['la_mask']
    
    val_self_mask_list = data_dict['val']['self_mask']
    val_la_mask_list = data_dict['val']['la_mask']

    # =========================================================================
    # dataloader 생성
    print('train dataloader 생성 중...')
    Train_Dataloader = dam.get_dataloader(
        x_list=x_train_ary_ary,
        y_list=y_train_ary_ary,
        x_mark_list=copy.deepcopy(mark_train_ary_ary),
        y_mark_list=copy.deepcopy(mark_train_ary_ary),
        self_mask_list=train_self_mask_list,
        la_mask_list=train_la_mask_list,
    ...
    Val_Dataloader = dam.get_dataloader(
        x_list=x_val_ary_ary,
        y_list=y_val_ary_ary,
        x_mark_list=copy.deepcopy(mark_val_ary_ary),
        y_mark_list=copy.deepcopy(mark_val_ary_ary),
    ...
    enc_feature_len = x_train_ary_ary[0].shape[-1]
    dec_feature_len = y_train_ary_ary[0].shape[-1]
    enc_temporal_len = mark_train_ary_ary[0].shape[-1]
    dec_temporal_len = mark_train_ary_ary[0].shape[-1]
    ...
    model = net.VnAW(
        ...
        embed_type=embed_type, d_model=d_model, 
    ...
    # scheduler
    total_iter = epochs * (1 + len(x_train_ary_ary) // batch_size)
