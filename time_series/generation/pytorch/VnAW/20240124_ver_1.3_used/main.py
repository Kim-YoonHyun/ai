...
if __name__ == "__main__":
    ...
    x_mark_train_name_list = os.listdir(f'{dataset_path}/train/x_mark')
    x_mark_train_name_list.sort()
    
    y_train_name_list = os.listdir(f'{dataset_path}/train/y')
    y_train_name_list.sort()
    y_mark_train_name_list = os.listdir(f'{dataset_path}/train/y_mark')
    y_mark_train_name_list.sort()
    
    x_val_name_list = os.listdir(f'{dataset_path}/val/x')
    x_val_name_list.sort()
    x_mark_val_name_list = os.listdir(f'{dataset_path}/val/x_mark')
    x_mark_val_name_list.sort()
    
    y_val_name_list = os.listdir(f'{dataset_path}/val/y')
    y_val_name_list.sort()
    y_mark_val_name_list = os.listdir(f'{dataset_path}/val/y_mark')
    y_mark_val_name_list.sort()
    ...
    x_mark_train_df_list = []
    for x_mtn in x_mark_train_name_list:
        x_mark_train_df = pd.read_csv(f'{dataset_path}/train/x_mark/{x_mtn}')
        x_mark_train_df_list.append(x_mark_train_df)
    # train y
    y_train_df_list = []
    for y_tn in y_train_name_list:
        y_train_df = pd.read_csv(f'{dataset_path}/train/y/{y_tn}')
        y_train_df_list.append(y_train_df)
    y_mark_train_df_list = []
    for y_mtn in y_mark_train_name_list:
        y_mark_train_df = pd.read_csv(f'{dataset_path}/train/y_mark/{y_mtn}')
        y_mark_train_df_list.append(y_mark_train_df)
    # validation x
    x_val_df_list = []
    ...
    x_mark_val_df_list = []
    for x_mvn in x_mark_val_name_list:
        x_mark_val_df = pd.read_csv(f'{dataset_path}/val/x_mark/{x_mvn}')
        x_mark_val_df_list.append(x_mark_val_df)
    # validation y
    y_val_df_list = []
    ...
    y_mark_val_df_list = []
    for y_mvn in y_mark_val_name_list:
        y_mark_val_df = pd.read_csv(f'{dataset_path}/val/y_mark/{y_mvn}')
        y_mark_val_df_list.append(y_mark_val_df)
    ...
    Train_Dataloader = dam.get_dataloader(
        ...
        x_mark_df_list=x_mark_train_df_list,
        y_df_list=y_train_df_list,
        y_mark_df_list=y_mark_train_df_list,
        pred_len=pred_len,
    ...
    Val_Dataloader = dam.get_dataloader(
        ...
        x_mark_df_list=x_mark_val_df_list,
        y_df_list=x_val_df_list,
        y_mark_df_list=y_mark_val_df_list,
        pred_len=pred_len,
        batch_size=batch_size, 
        shuffle=shuffle, 
        drop_last=drop_last, 
    ...
    enc_temporal_len = len(x_mark_train_df_list[0].columns.values)
    
    dec_feature_len = len(y_train_df_list[0].columns.values)
    dec_temporal_len = len(y_mark_train_df_list[0].columns.values)
