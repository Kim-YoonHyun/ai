...
def xmy_preprocess(mode, whole_df, col_list, onoff_df=None, yesterday=None, col_max=None, col_min=None):
    if mode == 'diesel_new_제네레이터':
        col_df = Voltage_preprocessing(whole_df)
    elif mode == 'diesel_new_냉각수온도':
        col_df = Coolant_preprocessing_diesel(whole_df)
    elif mode == 'diesel_new_DPF':
        col_df = ECU_preprocessing(whole_df)
    elif mode == 'diesel_new_SCR':
        col_df = SCR_preprocessing(whole_df)
    elif mode in ['CNG_제네레이터', 'diesel_old_제네레이터']:
        col_df = Voltage_preprocessing_cng(whole_df)
    elif mode in ['CNG_냉각수온도', 'diesel_old_냉각수온도']:
        col_df = Coolant_preprocessing_cng(whole_df)
    # elif mode == 'CNG_실린더실화':
    #     print(mode)
    elif mode == 'CNG_연료탱크':
        col_df = FuelTankPressure_preprocessing(whole_df)
    elif mode in ['x', 'mark']:
        ...
        raise KeyError(f'연료타입이 잘못되었습니다. {mode}')
...
def Coolant_preprocessing_diesel(df):
    # group_size 보다 작은 valid행은 1->0, 0->1로 변환 
    group_size = 300
    # threshold 이상 모든 열이 연속된 결측치를 검사
    threshold = 30
...
def Coolant_preprocessing_cng(df):
    # group_size 보다 작은 valid행은 1->0, 0->1로 변환 
    group_size = 300
    # threshold 이상 모든 열이 연속된 결측치를 검사
    threshold = 30
    # 기준 엔진속도 
    speed = 480
    colums = ['ADC_ACC','ECU_CoolantTemperature','ECU_EngineSpeed']
    columns_to_interpolate = ['ECU_CoolantTemperature','ECU_EngineSpeed']
    df['valid'] = np.nan
    mask = ((df['ADC_ACC'] == 1))
    ...
    # ADC_ACC가 0인 행 -> valid 0
    df.loc[df['ADC_ACC']==0, 'valid'] = 0
    df.loc[df['ECU_EngineSpeed']<490, 'valid'] = 0
    # 30행 이상 모든 열이 연속된 결측치인 경우 -> valid 0
    for column in colums:
        non_nan_indices = df[column].notnull().astype(int)
        non_nan_indices_group = non_nan_indices.diff().ne(0).cumsum()
    ...
    # 센서 상한과 하한을 기준으로 1차 전처리 
    df.loc[(df['ECU_CoolantTemperature'] > 200)|(df['ECU_CoolantTemperature'] < -40), 'ECU_CoolantTemperature'] = np.nan
    df.loc[(df['ECU_EngineSpeed'] > 5000)|(df['ECU_EngineSpeed'] < 0), 'ECU_EngineSpeed'] = np.nan

    
    # # 냉각수 온도가 이전 값과 5 이상 차이나는 경우 np.nan으로 변경
    df['ECU_CoolantTemperature'] = np.where(
    abs(df['ECU_CoolantTemperature'].fillna(method='ffill') - df['ECU_CoolantTemperature'].fillna(method='ffill').shift(1)) >= 5,
    np.where(
        df['ECU_CoolantTemperature'].notna(),  # 원래 NaN이 아닌 값을 유지하기 위한 조건
        np.nan,  # 조건을 만족하면 NaN으로 설정
        df['ECU_CoolantTemperature']  # 원래 NaN인 값은 그대로 NaN으로 유지
    ),
    df['ECU_CoolantTemperature']  # 조건을 만족하지 않으면 원래 값을 유지
    )
    
    # ECU_CoolantTemperature 결측치 이전 값으로 보간
    df.loc[mask, 'ECU_CoolantTemperature'] = df.loc[mask, 'ECU_CoolantTemperature'].fillna(method='bfill')  
...
def Voltage_preprocessing_cng(df):
    # group_size 보다 작은 valid행은 1->0, 0->1로 변환 
    group_size = 300
    # threshold 이상 모든 열이 연속된 결측치를 검사
    threshold = 30
    ...
    colums = ['ADC_ACC','ECU_BatteryVoltage','ECU_EngineSpeed']
    columns_to_interpolate = ['ECU_BatteryVoltage','ECU_EngineSpeed']
    df['valid'] = np.nan
    for column in colums:  
        # 처음으로 값이 들어오는 데이터 전까지 결측치 확인
        first_valid_index = df.dropna(how='all').index[0] if not df.dropna(how='all').empty else None
    ...
    # ECU_BatteryVoltage 결측치 이전 값으로 보간
    mask = ((df['ADC_ACC'] == 1)&(df['ECU_EngineSpeed']>speed))
    df.loc[mask, 'ECU_BatteryVoltage'] = df.loc[mask, 'ECU_BatteryVoltage'].fillna(method='bfill')

    # 전압 95% 미만으로 떨어지는 경우 -> valid 0 (고장코드에서는 삭제)
    df.loc[((df['ECU_EngineSpeed']>=500)&(df['ECU_BatteryVoltage']<vol_safe)),'valid'] = 0
...
def FuelTankPressure_preprocessing(df):
    # group_size 보다 작은 valid행은 1->0, 0->1로 변환 
    group_size = 300
    # threshold 이상 모든 열이 연속된 결측치를 검사
    threshold = 30
    
    colums = ['ADC_ACC','ECU_FuelTankPressure']
    columns_to_interpolate = ['ECU_FuelTankPressure',]
    ...
    # 센서 상한과 하한을 기준으로 1차 전처리 
    df.loc[(df['ECU_FuelTankPressure'] > 500)|(df['ECU_FuelTankPressure'] < 0), 'ECU_FuelTankPressure'] = np.nan
    df.loc[(df['ECU_EngineSpeed'] > 5000)|(df['ECU_EngineSpeed'] < 0), 'ECU_EngineSpeed'] = np.nan

    
    # 연료탱크 압력이 이전 값과 5 이상 차이나는 경우 np.nan으로 변경
    df['ECU_FuelTankPressure'] = np.where(
    abs(df['ECU_FuelTankPressure'].fillna(method='ffill') - df['ECU_FuelTankPressure'].fillna(method='ffill').shift(1)) >= 10,
    np.where(
        df['ECU_FuelTankPressure'].notna(),  # 원래 NaN이 아닌 값을 유지하기 위한 조건
        np.nan,  # 조건을 만족하면 NaN으로 설정
        df['ECU_FuelTankPressure']  # 원래 NaN인 값은 그대로 NaN으로 유지
    ),
    df['ECU_FuelTankPressure']  # 조건을 만족하지 않으면 원래 값을 유지
    )

                    
    # ECU_FuelTankPressure 결측치 이전 값으로 보간
    df.loc[mask, 'ECU_FuelTankPressure'] = df.loc[mask, 'ECU_FuelTankPressure'].fillna(method='ffill')  
    df.loc[mask, 'ECU_FuelTankPressure'] = df.loc[mask, 'ECU_FuelTankPressure'].interpolate(method='linear')
    # 나머지 행 -> valid 1
    df.loc[df['valid']!=0, 'valid'] = 1
