...
# def train_val_test_df_separate(df, class_label_list, train_p=0.8):
#     '''
...
# def make_json_dataset(load_path):
#     '''
...
#     class_list = np.unique(string_class_list).tolist()
#     class_dict = utils.make_class_dict(class_list)
#     new_dataset['class_dict'] = class_dict
#     label_list = list(class_dict.values())

#     string_label_ary = np.array(string_class_list.copy())
#     for class_name, label in zip(class_list, label_list):
#         string_label_ary = np.where(string_label_ary==class_name, label, string_label_ary)
#     string_label_list = list(map(int, string_label_ary))

#     # df 화
#     df = pd.DataFrame([string_list, string_class_list, string_label_list], index=['string', 'class', 'label']).T
    
#     # train df, val df 로 분리
#     train_df, val_df = train_val_test_df_separate(df, class_list, train_p=0.8)
    
#     idx = 0
#     for tvt, tvt_str in zip([train_df, val_df], ['train_data', 'validation_data']):
#         # 전체 갯수 추가
...
#             if idx == 0:
#                 new_dataset['number']['total'][1][class_name] = 0
            
#             # 특정 라벨 만 추출
#             df_tvt_l = tvt[tvt['class'] == class_name]
            
#             # 데이터 전체 기준 갯수 추가
#             new_dataset['number']['total'][1][class_name] += len(df_tvt_l)
            
#             # train, val, test 기준 전체 갯수 추가
#             new_dataset['number'][tvt_str]['total'] += len(df_tvt_l)
            
#             # train, val, test 기준 각 라벨 갯수 추가
#             new_dataset['number'][tvt_str][class_name] = len(df_tvt_l)
