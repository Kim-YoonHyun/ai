...
def main():
    ...
    tokenizer = dm.get_kobert_tokenizer(pre_trained='skt/kobert-base-v1')
    
    log.info('>>> get vocab')
    vocab = dm.get_vocab(tokenizer)
    ...
    model = mm.get_bert_model(method='BertForSequenceClassification', pre_trained="skt/kobert-base-v1", num_labels=len(uni_class_list))
    ...
    # -------------------------------------------------------------------------
    log.info('>>> model test')
    pred_label_ary, pred_reliability_ary, pred_2nd_label_ary, true_label_ary = tutils.model_test(
        model=model,
    ...
    pred_class_list = []
    pred_2nd_class_list = []
    for true_label, pred_label, pred_2nd_label in zip(true_label_ary, pred_label_ary, pred_2nd_label_ary):
        true_class = uni_class_list[true_label]
        pred_class = uni_class_list[pred_label]
        pred_2nd_class = uni_class_list[pred_2nd_label]
