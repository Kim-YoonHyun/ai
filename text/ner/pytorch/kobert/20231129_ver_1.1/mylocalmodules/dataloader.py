...
def get_feature(whole_string_split_list, whole_id_split_list, tokenizer, max_seq_len,
    ...
    for string_split_list, id_split_list in zip(whole_string_split_list, whole_id_split_list):
        ...
        # print(vars(tokenizer)['_bos_token'])
        # print(vars(tokenizer)['_eos_token'])
        # print(vars(tokenizer)['_unk_token'])
        # print(vars(tokenizer)['_sep_token'])
        # print(vars(tokenizer)['_pad_token'])
        # print(vars(tokenizer)['_cls_token'])
        # print(vars(tokenizer)['_mask_token'])
        # print(vars(tokenizer)['_pad_token_type_id'])
        # print(vars(tokenizer)['_additional_special_tokens'])
        
        # print(list(vars(tokenizer)['token2idx'].keys())[:10])
        # print(list(vars(tokenizer)['token2idx'].values())[:10])
        # a = vars(tokenizer)['token2idx']
        # print(len(a))
        # with open('./kobert_token2idx_dict.json', 'w', encoding='utf-8-sig') as f:
        #     json.dump(a, f, indent='\t', ensure_ascii=False)
        # ==
...
# 너무 긴 문장 제어   
def separate_long_text(string_split_list, idx_list, stan_num): 
    b_e_list = []
    new_test_idx_list = []
    test_string_split_list = []
    for test_string_split, test_idx in zip(string_split_list, idx_list):
