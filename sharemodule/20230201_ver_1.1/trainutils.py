...
def get_output(mode, dataloader, model, device, loss_function=None, optimizer=None, scheduler=None, amp=None, max_grad_norm=None):
    '''
    ...
    Pred_Iterator = tm.Iterator(dataloader, model, device)

    n = 0
    for pred, b_label in tqdm(Pred_Iterator):
        n += 1
    # batch 입력
    #^^^^^^^^^^^^^^^^^^^^^^^^
    # for batch_idx, (x, b_label) in enumerate(tqdm(dataloader)):
    #     if np.shape(x)[0] == 1:
    #         continue
    #     # 각 변수를 device 에 올리기
    ...
    running_loss = loss_sum / n
    return pred_label_ary, pred_reliability_ary, pred_2nd_label_ary, true_label_ary, running_loss
